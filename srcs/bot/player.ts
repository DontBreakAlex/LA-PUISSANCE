...
class Play extends Command {
	...
	async execute(message: Message, array: string[]): Promise<void> {
		if (!message.guild || !message.member || message.channel instanceof DMChannel || message.channel instanceof NewsChannel) 
			return;
		if (!array[1]) 
			return message.guild.status.queue.play(message.channel, message.member);
		let player: Player;
...
class PlayerQueue extends Array<Player> {
	...
	async play(channel: TextChannel, member: GuildMember): Promise<void> {
		if (this.parent.dispatcher?.paused) 
			this.parent.dispatcher.resume();
		if (this.parent.playing == Playing.None && this.length != 0) {
			...
			while (this.length != 0) {
				const Player = <Player>this.shift();
				try {
